...
class ImageEntitySpec: QuickSpec {
    override func spec() {
        it("parses JSON data to create a new instance.") {
            let image: ImageEntity? = try? ImageEntity.decodeValue(imageJSON)
        ...
        it("gets an empty array if tags element is nil.") {
            ...
            let image: ImageEntity? = try? ImageEntity.decodeValue(missingJSON)
        ...
        it("ignores an extra JOSN element.") {
            ...
            let image: ImageEntity? = try? ImageEntity.decodeValue(extraJSON)
